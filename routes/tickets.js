...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    if (user.role === 'customer') {
      ...
      // Upsert customer profile (name, email, location from registration)
      const {
      ...
      if (firstName && lastName && country && city) {
        const profEmail = customerEmail || email || null;
        await db.query(
          `INSERT INTO customer_profiles
            (phone, first_name, last_name, email, country, city, address)
           VALUES (?, ?, ?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE
             ...
             updated_at = NOW()`,
          [user.phone, firstName, lastName, profEmail, country, city, address || null]
        ).catch(err => console.warn('[profile upsert]', err.message));
      }
    }
...
// ─────────────────────────────────────────────────────────────
//  GET /api/tickets/cooldown?phone=+63XXXXXXXXXX
//  Check if a customer is in cooldown (no auth needed — phone is
//  the identifier, not sensitive data by itself)
// ─────────────────────────────────────────────────────────────
router.get('/cooldown', async (req, res) => {
  try {
    const { phone } = req.query;
    if (!phone) return res.json({ cooldown: false });

    const [rows] = await db.query(
      'SELECT submitted_at FROM ticket_cooldowns WHERE phone = ?',
      [phone]
    );
    if (!rows.length) return res.json({ cooldown: false });

    const elapsed   = Date.now() - new Date(rows[0].submitted_at).getTime();
    const remaining = 60 * 60 * 1000 - elapsed;
    if (remaining <= 0) return res.json({ cooldown: false });

    return res.json({ cooldown: true, remaining });
  } catch (err) {
    console.error('[GET /cooldown]', err);
...
//  Fetch a single ticket by its human-readable ID (e.g. TK-0001)
// ─────────────────────────────────────────────────────────────
router.get('/by-no/:ticketNo', requireEmployee, async (req, res) => {
  try {
    ...
    console.error('[GET /by-no]', err);
    return res.status(500).json({ error: 'Server error.' });
  }
});

// ─────────────────────────────────────────────────────────────
//  PATCH /api/tickets/by-no/:ticketNo/status
// ─────────────────────────────────────────────────────────────
router.patch('/by-no/:ticketNo/status', requireEmployee, async (req, res) => {
  try {
    const allowed = ['open','progress','resolved','closed'];
    const { status } = req.body;
    if (!allowed.includes(status)) {
    ...
    console.error('[PATCH /by-no/status]', err);
    return res.status(500).json({ error: 'Server error.' });
  }
});

// ─────────────────────────────────────────────────────────────
//  POST /api/tickets/by-no/:ticketNo/comments
// ─────────────────────────────────────────────────────────────
router.post('/by-no/:ticketNo/comments', requireEmployee, async (req, res) => {
  try {
    const { body } = req.body;
    if (!body || !body.trim()) {
    ...
    await db.query(
      'INSERT INTO comments (ticket_id, author, body) VALUES (?, ?, ?)',
      [rows[0].id, author, body.trim()]
    );
    return res.json({ success: true });
  } catch (err) {
    console.error('[POST /by-no/comments]', err);
    return res.status(500).json({ error: 'Server error.' });
  }
});
